...
def _process_audio(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "transcribe":
        try:
            ...
            # Use the existing local Whisper STT instead of Gemini Transcribe.
            ...
            if params.get("save", True):
                ...
                out.write_text(result, encoding="utf-8")
                return f"Whisper transcription saved: {out.name}\n\nPreview: {result[:300]}"
            return result
        except ImportError:
            return "Whisper transcription requires faster-whisper. Install it with: pip install faster-whisper"
        except Exception as e:
