...
def _read_full_config() -> dict:
    """Read local non-secret settings from the ignored config store."""
    return load_api_keys()


_DEFAULT_W, _DEFAULT_H = 1220, 780
...
class C:
    # Modern "Aurora Glass" palette — the HUD animation still uses these
    # values, so changing the theme here updates the whole application.
    BG        = "#050816"
    PANEL     = "#0a1020"
    PANEL2    = "#0e172b"
    BORDER    = "#1b2a46"
...
def glass_shadow(widget: QWidget, blur: int = 28, alpha: int = 120, y: int = 6) -> None:
    """Subtle soft bloom used by the 2078 glass UI without changing behaviour."""
    fx = QGraphicsDropShadowEffect(widget)
    fx.setBlurRadius(blur)
    fx.setOffset(0, y)
    fx.setColor(qcol(C.PRI, alpha))
...
def glass_style(
    *,
    ...
    # rgba() keeps the layers translucent while remaining fully Qt-style-sheet compatible.
    return f"background:rgba({int(fill[1:3],16)},{int(fill[3:5],16)},{int(fill[5:7],16)},{alpha}); border:1px solid {border}; border-radius:{radius}px;"
...
class HudCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        cx, cy = W / 2, H / 2 - min(H, W) * 0.015
        fw = min(W, H)
        muted = self.muted
        accent = C.MUTED_C if muted else C.PRI
        r = fw * 0.245 * self._scale

        # Subtle technical grid / starfield.
        p.setPen(QPen(qcol(C.PRI, 18), 1))
        step = max(44, int(fw * 0.07))
        ...
        random.seed(91)
        for _ in range(55):
            x, y = random.randrange(max(1, W)), random.randrange(max(1, H))
            p.setPen(QPen(qcol(C.PRI, random.randint(12, 34)), 1))
            p.drawPoint(x, y)

        # Ambient bloom behind the core.
        for i in range(14, 0, -1):
            rr = r * (1.15 + i * 0.10)
            alpha = int((15 - i + 1) * (2.6 if self.speaking else 1.5))
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(qcol(accent, max(2, min(34, alpha)))))
            p.drawEllipse(QRectF(cx - rr, cy - rr, rr * 2, rr * 2))

        # Thin orbital tracks — cleaner than the old radar rings.
        orbit_data = [
            (0.38, 2.0, self._rings[0], 78, 150),
            (0.47, 1.2, self._rings[1], 48, 105),
            (0.56, 1.0, self._rings[2], 32, 72),
        ]
        for frac, width, base, gap, arc_len in orbit_data:
            rr = fw * frac
            rect = QRectF(cx - rr, cy - rr, rr * 2, rr * 2)
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.setPen(QPen(qcol(C.PRI, 45), width))
            p.drawEllipse(rect)
            p.setPen(QPen(qcol(accent, 170), width + 0.6))
            angle = base
            while angle < base + 360:
                p.drawArc(rect, int(angle * 16), int(arc_len * 16))
                angle += arc_len + gap

        # Rotating data nodes on the orbits.
        for idx, rr_frac in enumerate((0.38, 0.47, 0.56)):
            rr = fw * rr_frac
            ang = math.radians(self._rings[idx] * (1 if idx != 1 else -1))
            nx = cx + rr * math.cos(ang)
            ny = cy + rr * math.sin(ang)
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(qcol(accent, 230)))
            p.drawEllipse(QPointF(nx, ny), 3.0 if idx == 0 else 2.0, 3.0 if idx == 0 else 2.0)

        # Main glass core: layered radial gradients.
        grad = QRadialGradient(QPointF(cx - r * 0.18, cy - r * 0.20), r * 1.05)
        grad.setColorAt(0.00, qcol(C.WHITE, 210 if self.speaking else 150))
        grad.setColorAt(0.08, qcol(accent, 235))
        grad.setColorAt(0.32, qcol(C.PANEL2, 245))
        grad.setColorAt(0.72, qcol(C.PANEL, 250))
        grad.setColorAt(1.00, qcol(C.DARK, 255))
        p.setPen(QPen(qcol(accent, 210), 2.0))
        p.setBrush(QBrush(grad))
        p.drawEllipse(QRectF(cx-r, cy-r, r*2, r*2))

        # Inner luminous ring + segmented perimeter.
        inner = r * 0.88
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.setPen(QPen(qcol(accent, 120), 1.2))
        p.drawEllipse(QRectF(cx-inner, cy-inner, inner*2, inner*2))
        p.setPen(QPen(qcol(accent, 215), 2.2))
        arc_rect = QRectF(cx-inner*1.06, cy-inner*1.06, inner*2.12, inner*2.12)
        p.drawArc(arc_rect, int(self._scan * 16), 92 * 16)
        p.setPen(QPen(qcol(C.ACC, 190), 1.6))
        p.drawArc(arc_rect, int(self._scan2 * 16), 54 * 16)
        ...
        if self._face_px:
            ...
            # Soft glass backing around the face.
            p.setPen(QPen(qcol(C.WHITE, 35), 1))
            p.setBrush(qcol(C.WHITE, 10))
            p.drawEllipse(QRectF(cx-fsz*0.52, cy-fsz*0.52, fsz*1.04, fsz*1.04))
            p.drawPixmap(int(cx-fsz/2), int(cy-fsz/2), scaled)
        else:
            # Fallback AI glyph.
            p.setPen(QPen(qcol(accent, 230), 2))
            p.setBrush(qcol(accent, 25))
            p.drawEllipse(QRectF(cx-r*0.48, cy-r*0.48, r*0.96, r*0.96))
            p.setFont(QFont("Segoe UI", max(16, int(fw*0.035)), QFont.Weight.Bold))
            p.drawText(QRectF(cx-r*0.7, cy-r*0.15, r*1.4, r*0.3),
                       Qt.AlignmentFlag.AlignCenter, self._assistant_name)

        # Minimal crosshair, tucked behind the core.
        p.setPen(QPen(qcol(accent, 80), 1))
        gap = r * 1.22
        reach = fw * 0.47
        ...
        # Small telemetry labels around the core.
        p.setFont(QFont("Segoe UI", max(7, int(fw*0.012)), QFont.Weight.DemiBold))
        p.setPen(QPen(qcol(C.TEXT_DIM, 210), 1))
        p.drawText(QRectF(cx-r*1.95, cy-r*0.78, r*0.8, 18),
                   Qt.AlignmentFlag.AlignLeft, "NEURAL LINK")
        p.drawText(QRectF(cx+r*1.15, cy+r*0.55, r*0.9, 18),
                   Qt.AlignmentFlag.AlignRight, "CORE ONLINE")
        p.setPen(QPen(qcol(accent, 170), 1))
        p.drawText(QRectF(cx-r*1.95, cy-r*0.78+16, r*0.8, 18),
                   Qt.AlignmentFlag.AlignLeft, "SYNC  99.8%")
        p.drawText(QRectF(cx+r*1.15, cy+r*0.55+16, r*0.9, 18),
                   Qt.AlignmentFlag.AlignRight, "SECURE")

        # Status pill.
        sy = cy + r * 1.35
        if muted:
            txt, col = "MIC OFF", qcol(C.MUTED_C)
        elif self.speaking:
            txt, col = "VOICE ACTIVE", qcol(C.ACC)
        ...
        pill_w, pill_h = min(220, fw*0.30), 30
        pill = QRectF(cx-pill_w/2, sy, pill_w, pill_h)
        p.setPen(QPen(qcol(col.name(), 130), 1))
        p.setBrush(qcol(C.PANEL2, 225))
        p.drawRoundedRect(pill, 15, 15)
        p.setPen(QPen(col, 1))
        p.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
        p.drawText(pill, Qt.AlignmentFlag.AlignCenter, "●  " + txt)

        # Modern mini waveform beneath the status.
        wy = sy + 43
        N = 30
        bw = max(3, min(7, fw * 0.007))
        total = N * bw
        wx0 = cx - total / 2
        for i in range(N):
            if muted:
                ...
                hgt = 4 + int((math.sin(self._tick*0.22 + i*0.7)+1)*7)
                cl = qcol(C.PRI if i % 3 else C.ACC, 210)
            else:
                hgt = 2 + int((math.sin(self._tick*0.06 + i*0.7)+1)*1.5)
                cl = qcol(C.BORDER_B, 150)
            p.setBrush(QBrush(cl)); p.setPen(Qt.PenStyle.NoPen)
            p.drawRoundedRect(QRectF(wx0+i*bw, wy+12-hgt, max(2,bw-2), hgt), 2, 2)

        # Active particles, now concentrated around the orb.
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle(f"{_display} — MARK XLIX / 2078")
        ...
        central.setStyleSheet(f"""
            QWidget {{
                background: qlineargradient(x1:0,y1:0,x2:1,y2:1,
                    stop:0 {C.BG}, stop:0.42 {C.DARK}, stop:1 {C.BG});
                color: {C.TEXT};
            }}
        """)
        ...
        _cam_cont = QWidget()
        _cam_cont.setStyleSheet(f"background:rgba(0,3,8,230); border:1px solid {C.BORDER_B};")
    ...
    def _build_header(self) -> QWidget:
        ...
        w.setStyleSheet(f"""
            QWidget {{
                background: rgba(7,12,24,205);
                border-bottom: 1px solid {C.BORDER_B};
            }}
        """)
        lay = QHBoxLayout(w)
        lay.setContentsMargins(18, 0, 20, 0)
        glass_shadow(w, 24, 80, 4)
        lay.setSpacing(10)

        _disp = self._assistant_name.upper()
        brand = QLabel(f"{_disp}  /  CONTROL")
        brand.setFont(QFont("Segoe UI", 9, QFont.Weight.DemiBold))
        brand.setStyleSheet(
        ...
        self._drawer_btn.setFixedSize(38, 38)
        self._drawer_btn.setFont(QFont("Segoe UI Symbol", 14))
        ...
        self._title_lbl.setFont(QFont("Segoe UI", 19, QFont.Weight.Bold))
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        w.setStyleSheet(
            f"background:rgba(7,12,24,188); border-right:1px solid {C.BORDER_B};"
        )
        glass_shadow(w, 34, 90, 0)
        lay = QVBoxLayout(w)
        lay.setContentsMargins(14, 14, 14, 14)
        lay.setSpacing(9)

        hdr = QLabel("SYSTEM  /  LIVE")
        hdr.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold))
        hdr.setStyleSheet(
            f"color:{C.TEXT_MED}; background:transparent; "
            f"letter-spacing:1px; padding-bottom:5px;"
        )
        lay.addWidget(hdr)
        ...
        self._bar_tmp = MetricBar("TEMP", "#fb7185")
        ...
        info_panel.setStyleSheet(f"""
            QWidget {{
                background:rgba(14,23,43,150);
                border:1px solid {C.BORDER_B};
                border-radius:16px;
            }}
        """)
        ip_lay = QVBoxLayout(info_panel)
        ip_lay.setContentsMargins(12, 11, 12, 11)
        ip_lay.setSpacing(6)
        ...
        os_lbl = QLabel(f"PLATFORM   {os_name}")
        ...
        for txt, col in [
            ...
            ("SECURITY   READY", C.PRI),
            ("MARK XLIX   /   49", C.TEXT_DIM),
        ]:
            card = QLabel(txt)
            card.setFont(QFont("Segoe UI", 7, QFont.Weight.DemiBold))
            card.setAlignment(Qt.AlignmentFlag.AlignCenter)
            card.setStyleSheet(f"""
                QLabel {{
                    color:{col};
                    background:rgba(14,23,43,135);
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        w.setStyleSheet(
            f"background:rgba(7,12,24,188); border-left:1px solid {C.BORDER_B};"
        )
        glass_shadow(w, 34, 90, 0)
        lay = QVBoxLayout(w)
        lay.setContentsMargins(14, 14, 14, 14)
        ...
        lay.addWidget(_sec("ACTIVITY", "LIVE"))
        self._log = LogWidget()
        ...
        lay.addWidget(_sec("ATTACHMENTS", "DROP / BROWSE"))
        self._drop_zone = FileDropZone()
        self._drop_zone.file_selected.connect(self._on_file_selected)
        lay.addWidget(self._drop_zone)

        self._file_hint = QLabel("No file selected")
        self._file_hint.setFont(QFont("Segoe UI", 7))
        self._file_hint.setStyleSheet(
            f"color:{C.TEXT_DIM}; background:transparent; padding:2px 3px;"
        )
        ...
        lay.addWidget(_sec("COMMAND CENTER", "READY"))
        ...
        self._interrupt_btn.setStyleSheet(f"""
            ...
            QPushButton:pressed {{ background:rgba(251,113,133,0.20); }}
        """)
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        _BTN_STYLE_PRI = f"""
            QPushButton {{
                ...
                text-align: left; padding: 0 11px;
            }}
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input.setPlaceholderText("Ask AVEN anything…")
        self._input.setFont(QFont("Segoe UI", 9))
        self._input.setFixedHeight(40)
        self._input.setStyleSheet(f"""
            QLineEdit {{
                background:rgba(10,16,32,205);
                color:{C.WHITE};
                border:1px solid {C.BORDER_B};
                border-radius:14px;
                padding:0 12px;
    ...
    def _build_footer(self) -> QWidget:
        ...
        w.setFixedHeight(30)
        w.setStyleSheet(
            f"background:rgba(7,12,24,210); border-top:1px solid {C.BORDER_B};"
        )
        lay = QHBoxLayout(w)
        ...
        lay.addStretch()
        lay.addWidget(_fl("AVEN  •  SECURE SESSION", C.PRI_DIM))
        return w
    ...
    def _on_setup_done(self, key: str, os_name: str):
        save_api_keys(key)
        save_setting("os_system", os_name)
        self._ready = True
        if self._overlay:
            self._overlay.hide()
            self._overlay = None
