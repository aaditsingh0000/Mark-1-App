...
def _trim_to_limit(memory: dict) -> dict:
    """Keep the local memory store compact without discarding the schema."""
    if len(json.dumps(memory, ensure_ascii=False)) <= MEMORY_MAX_CHARS:
        return memory

    # Sessions are append-only and otherwise could bypass the normal entry trim.
    sessions = memory.get("sessions")
    if isinstance(sessions, list):
        while sessions and len(json.dumps(memory, ensure_ascii=False)) > MEMORY_MAX_CHARS:
            sessions.pop(0)
...
def search_memory(query: str) -> str:
    """Search all remembered values without exposing the full store."""
    q = (query or "").strip().lower()
    if not q:
        return "Provide a memory search query."
    memory = load_memory()
    hits = []
    for category, values in memory.items():
        ...
        for key, entry in values.items():
            ...
            if q in f"{category} {key} {val}".lower():
                hits.append(f"{category}/{key}: {val}")
    return "\n".join(hits[:30]) if hits else "No matching memories found."


def memory_overview() -> str:
    """Return a compact memory inventory."""
    memory = load_memory()
    rows = []
    for category, values in memory.items():
        if isinstance(values, dict) and values:
            rows.append(f"{category}: {len(values)}")
